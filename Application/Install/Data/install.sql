...
INSERT INTO `opensns_advertising` (`id`, `title`, `type`, `width`, `height`, `status`, `pos`) VALUES
...
(64, 'FOOTER_QCODE', 2, '底部二维码部分代码', 1, '', '', 1408008644, 1408008738, 1, ' <img src="/Public/Core/images/qrcode.png"/>', 11),
...
(1, '默认版块', 1407114174, 10, 1, '1', 0, 0, 0, '', '', 1, 0);
...
(7, 1, 'OpenSNS', 92, 'http://tox.ourstu.com', 0, 1, 1407156830);
...
ALTER TABLE  `opensns_shop` CHANGE  `goods_detail`  `goods_detail` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT  '商品详情';

ALTER TABLE  `opensns_file` ADD  `driver` VARCHAR( 50 ) NOT NULL;
ALTER TABLE  `opensns_advertising` ADD  `s` TINYINT NOT NULL;
...
INSERT INTO `opensns_module` ( `name`, `alias`, `version`, `is_com`, `show_nav`, `summary`, `developer`, `website`, `entry`, `is_setup`, `sort`) VALUES
( 'Blog', '资讯', '1.0.0', 0, 1, '原OneThink的内容模块，OpenSNS实现了前台界面，传统的CMS模块', '上海顶想信息科技有限公司', 'http://www.topthink.net/', 'Blog/index/index', 1, 0),
...
/*修改等级设置为基础设置*/
UPDATE  `opensns_menu` SET  `title` =  '基础设置',
`url` =  'User/config' WHERE  `opensns_menu`.`url` ='User/level';
/*修改用户组名称*/
UPDATE  `opensns_auth_group` SET  `title` =  '普通用户' WHERE  `opensns_auth_group`.`id` =1;
...
ALTER TABLE  `opensns_group_type` ADD  `pid` INT NOT NULL;

INSERT INTO `opensns_hooks` ( `name`, `description`, `type`, `update_time`) VALUES
( 'dealPicture', '上传图片处理', 2, 1417139975);


DELETE FROM `opensns_config` WHERE `opensns_config`.`name` = '_EXPRESSION_EXPRESSION';
DELETE FROM `opensns_menu` WHERE `opensns_menu`.`group` = '表情配置';
...
ALTER TABLE  `opensns_forum` ADD  `background` INT NOT NULL;
ALTER TABLE  `opensns_forum` ADD  `description` VARCHAR( 5000 ) NOT NULL;
ALTER TABLE  `opensns_forum` ADD  `admin` VARCHAR( 100 ) NOT NULL;
